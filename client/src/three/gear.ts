...
// --- Hair and beards --------------------------------------------------------
//
// THE FIRST THING THAT MAKES TWO CHARACTERS DIFFERENT PEOPLE.
//
// The game already tints each body from its owner's name, and that note is
// worth reading — it took two attempts to make four characters look like four
// characters rather than four shades of the same one. But a tint is a cast over
// the same silhouette: at the distance the camera sits, two players with
// different skin tones and identical outlines still read as the same figure
// wearing different lighting.
//
// Hair is the cheapest thing that changes the SILHOUETTE, which is what the eye
// actually sorts people by, and a beard is the cheapest thing that changes a
// face that has no features to change.
//
...
function hairGeometry(style: HairStyle): THREE.BufferGeometry | null {
  ...
  if (style === "crop") {
    // Close to the skull, with a fringe over the brow. The smallest possible
    // change and still unmistakably a different person in silhouette.
    return merge([
      dome(HEAD_HALF_WIDTH + 3, [0, brow - 4, -4], [1, 0.86, 1.12]),
      box([HEAD_HALF_WIDTH * 1.55, 9, 7], [0, brow + 1, HEAD_FRONT_Z - 5]),
    ]);
  }

  if (style === "mane") {
    // Long and heavy: a cap, a fall down the back and a sheet either side of
    // the face. This is the one that changes the outline most.
    return merge([
      dome(HEAD_HALF_WIDTH + 5, [0, brow - 6, -5], [1, 1.05, 1.15]),
      drape([
        { y: brow - 6, z: HEAD_BACK_Z + 14, halfWidth: 32 },
        { y: brow - 34, z: HEAD_BACK_Z + 8, halfWidth: 34 },
        { y: brow - 62, z: HEAD_BACK_Z + 2, halfWidth: 30 },
        { y: brow - 82, z: HEAD_BACK_Z - 2, halfWidth: 22 },
      ]),
      box([7, 46, 20], [HEAD_HALF_WIDTH + 1, brow - 26, 2]),
      box([7, 46, 20], [-HEAD_HALF_WIDTH - 1, brow - 26, 2]),
    ]);
  }

  if (style === "topknot") {
    // Shaved at the sides with a knot on the crown — the one that reads most
    // clearly from directly above, which is where this game's camera is.
    return merge([
      shell(HEAD_HALF_WIDTH + 2, HEAD_HALF_WIDTH + 3, 16, [0, brow - 10, -4], 1.1),
  ...
  return merge([
    dome(HEAD_HALF_WIDTH + 4, [0, brow - 5, -5], [1, 0.95, 1.12]),
    shell(9, 12, 16, [0, brow - 18, HEAD_BACK_Z + 2], 1),
    drape([
      { y: brow - 24, z: HEAD_BACK_Z - 2, halfWidth: 11 },
      { y: brow - 52, z: HEAD_BACK_Z - 8, halfWidth: 13 },
      { y: brow - 76, z: HEAD_BACK_Z - 12, halfWidth: 8 },
    ]),
  ]);
}

function beardGeometry(style: BeardStyle): THREE.BufferGeometry | null {
  const mid = (HEAD_TOP + HEAD_BOTTOM) / 2;
  const jaw = mid - 22;
  if (style === "none") return null;

  if (style === "stubble") {
    // A shadow along the jaw. Barely geometry, and it is the difference
    // between a face and a blank.
    return merge([
      shell(HEAD_HALF_WIDTH - 2, HEAD_HALF_WIDTH - 6, 14, [0, jaw + 4, 6], 1.05),
    ]);
  }

  if (style === "full") {
    return merge([
      shell(HEAD_HALF_WIDTH - 1, HEAD_HALF_WIDTH - 9, 22, [0, jaw, 6], 1.05),
      drape([
        { y: jaw - 8, z: HEAD_FRONT_Z - 6, halfWidth: 20 },
        { y: jaw - 26, z: HEAD_FRONT_Z - 8, halfWidth: 17 },
        { y: jaw - 40, z: HEAD_FRONT_Z - 12, halfWidth: 10 },
      ]),
      // The moustache, without which a beard reads as a chinstrap.
      box([26, 7, 8], [0, mid - 8, HEAD_FRONT_Z - 2]),
    ]);
  }

  // "braided": a full beard gathered into two falls, which at this size is the
  // most legible "this is a different person" a chin can carry.
  return merge([
    shell(HEAD_HALF_WIDTH - 1, HEAD_HALF_WIDTH - 8, 20, [0, jaw, 6], 1.05),
    box([24, 7, 8], [0, mid - 8, HEAD_FRONT_Z - 2]),
    shell(7, 5, 34, [-11, jaw - 26, HEAD_FRONT_Z - 10], 1),
    shell(7, 5, 34, [11, jaw - 26, HEAD_FRONT_Z - 10], 1),
    dome(6, [-11, jaw - 44, HEAD_FRONT_Z - 10], [1, -1.4, 1]),
    dome(6, [11, jaw - 44, HEAD_FRONT_Z - 10], [1, -1.4, 1]),
  ]);
}

/**
 * Hair and beard as gear attachments, so they ride the head bone exactly as a
 * helm does and need no separate plumbing.
 *
 * Coloured by a tint passed in rather than by rarity: hair is not loot.
 */
export function buildHairAndBeard(
  hair: HairStyle,
  beard: BeardStyle,
  tint: THREE.Color,
): GearAttachment[] {
  ...
  for (const [name, geometry] of [
    ["hair", hairGeometry(hair)],
    ["beard", beardGeometry(beard)],
  ] as [string, THREE.BufferGeometry | null][]) {
    if (!geometry) continue;
    const material = new THREE.MeshStandardMaterial({
      color: tint,
      roughness: 0.92,
      metalness: 0,
      // Hair drapes are single sheets, like a cape: without this the inside of
      // every fall is a hole when seen from behind.
      side: THREE.DoubleSide,
    });
    const mesh = new THREE.Mesh(geometry, material);
    mesh.name = `look_${name}`;
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    out.push({ bone: "Head", object: mesh });
  }
  return out;
}
