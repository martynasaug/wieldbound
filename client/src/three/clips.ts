 ...
 *
 * THE LAST ONE IS OURS. `Monk_Gather.glb` is generated by
 * `tools/art/monk_clips.py` and carries Chop, Mine and Pick — the three
 * animations no pack in this project has ever contained, because nobody in a
 * character pack ever swung an axe at a tree. It holds no mesh: a clip needs a
 * skeleton to be expressed against and nothing more, and it is authored on the
 * Monk's own bones so it plays on the body the game already wears.
 *
 * That is the answer to a question three milestones went the long way round:
 * gathering played the equipped weapon's attack, then one borrowed sword swing
 * for all three kinds, then bone arcs posed by hand every frame. All three were
 * ways of not having the clip. Now there is a clip.
 *
  ...
  loading = (async () => {
    ...
    for (const name of SOURCES) {
      ...
      try {
        ...
        //
        // BUT IT SAYS SO NOW. Silence here cost a debugging cycle: a newly
        // generated clip file is not served until the dev server restarts —
        // Vite leaves `public/models` unwatched deliberately, see
        // client/vite.config.ts — so the fetch returned an HTML page, the
        // parse threw, this swallowed it, and gathering quietly fell back to
        // posing bones by hand. Every symptom pointed at the animation code and
        // none of them at a 200 that was not a model.
        console.warn(`clips: ${name} did not load; its animations are missing`, err);
        continue;
      }
